...
class SearchRateLimiter:
    ...
    async def wait_if_needed(self) -> None:
        ...
        if len(self.request_times) >= self.max_per_minute:
            ...
            logger.info("Search API rate limit reached, waiting", wait_seconds=wait_time, current_requests=len(self.request_times))
...
class UserRepositoryDiscoverer:
    ...
    async def _discover_from_authored_prs(self, username: str, start_date: datetime, end_date: datetime) -> Set[str]:
        ...
        logger.info("Searching for authored PRs", username=username, query=query, results_count=len(results))
    ...
    async def _discover_from_reviewed_prs(self, username: str, start_date: datetime, end_date: datetime) -> Set[str]:
    ...
    async def _discover_from_issues(self, username: str, start_date: datetime, end_date: datetime) -> Set[str]:
    ...
    async def _discover_from_commits(self, username: str, start_date: datetime, end_date: datetime) -> Set[str]:
    ...
    async def _search_issues(self, query: str, per_page: int = 100) -> List[Dict[str, Any]]:
        ...
        while True:
            try:
                response = self.client.rest.search.issues_and_pull_requests(q=query, per_page=per_page, page=page, sort="created", order="desc")
                ...
                if page > 10:
                    logger.warning("Reached page limit for search", query=query, pages_fetched=page - 1, results_count=len(all_results))
                    break
                ...
                if e.response.status_code == 403:
                    ...
                    logger.warning("User not found on GitHub (normal for some LDAP users)", query=query, error=str(e))
                    # Extract username from query for the exception message
                    username = (
                        query.split("author:")[1].split()[0]
                        if "author:" in query
                        else query.split("reviewed-by:")[1].split()[0]
                ...
                logger.error("Unexpected error during search", query=query, error=str(e))
                break
    ...
    async def _search_commits(self, query: str, per_page: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            response = self.client.rest.search.commits(q=query, per_page=per_page, sort="committer-date", order="desc")
    ...
    def _extract_repo_from_url(self, url: str) -> str | None:
        ...
        try:
            ...
            logger.warning("Could not extract repo from URL", url=url, error=str(e))
